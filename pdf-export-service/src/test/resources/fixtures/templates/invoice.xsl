          ...
          <fo:table-row height="2cm" border-bottom-color="black" border-bottom-style="solid" border-bottom-width="0.5pt">
            <fo:table-cell padding-bottom="3pt" >
              <fo:block text-align="left" >
                <xsl:if test="document_meta/logo_filename">
                  <fo:external-graphic content-width="6.0cm">
                    <xsl:attribute name="src"><xsl:value-of select="document_meta/logo_filename"/></xsl:attribute>
                  </fo:external-graphic>
                </xsl:if>
              </fo:block>
            </fo:table-cell>
	...
	<fo:table-row >
	      ...
	      margin-bottom="0.5cm"><xsl:value-of select="document_meta/addresser"/></fo:block>
            ...
            <xsl:choose>
              ...
              <xsl:if test="commercial_document/party/contact">
              <fo:block font-size="9pt"
              font-family="BitstreamVeraSans"
              text-align="left">
       ...
       <fo:block font-size="15pt"
                 ...
                 font-weight="bold"
                 margin-top="2cm">
        Rechnung <xsl:value-of select="commercial_document/extra/iteration_number"/> - <xsl:value-of select="commercial_document/description"/>
       </fo:block>
       <xsl:for-each select="commercial_document/text_paragraphs/text_paragraph">
          <xsl:choose>
          ...
          <xsl:sort select="position_number" data-type="number"/>
             ...
             <fo:table-row keep-together="always" keep-with-previous="always">
                ...
                <fo:table-cell number-columns-spanned="2"
                   ...
                   <fo:block  text-align="end" font-size="8pt">
                      <xsl:choose>
                         <xsl:when test="not(commercial_document/last_calculated_price)">-</xsl:when>
                         <xsl:otherwise>
